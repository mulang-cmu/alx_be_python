...
class EBook(Book):
    ...
    def __str__(self):
        return f"{type(self).__name__}: {self.title} by {self.author}, File Size: {self.file_size}"
...
class Library:
    ...
    #         if type(book) == EBook:
    #             print(f"{type(book).__name__}: {book.title} by {book.author}, File Size: {book.file_size}")

    #         elif type(book) == PrintBook:
    #             print(f"{type(book).__name__}: {book.title} by {book.author}, Page Count: {book.page_count}")
